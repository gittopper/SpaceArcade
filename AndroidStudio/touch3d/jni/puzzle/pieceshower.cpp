...
namespace
{
void drawSquare(Renderer& renderer, const Vector& shift, const Vector& v1, const Vector& v2, const Vector& v3, const Vector& v4, const Vector& n, const Geometry::Vector& color)
{
    ...
    renderer.render(reinterpret_cast<GLfloat*>(vertices.data()), reinterpret_cast<GLfloat*>(colors.data()), reinterpret_cast<GLushort*>(indices.data()), 6);
}

void drawTriangle(Renderer& renderer, const Vector& shift, const Vector& v1, const Vector& v2, const Vector& v3, const Vector& n, const Geometry::Vector& color)
{
    std::vector<Geometry::Vector> vertices;
    vertices.push_back(Geometry::Vector(shift[0] + v1[0], shift[1] + v1[1], shift[2] + v1[2]));
    vertices.push_back(Geometry::Vector( shift[0] + v2[0], shift[1] + v2[1], shift[2] + v2[2]));
    ...
    std::vector<Geometry::Vector> colors = {color, color, color, color};
    renderer.render(reinterpret_cast<GLfloat*>(vertices.data()), reinterpret_cast<GLfloat*>(colors.data()), reinterpret_cast<GLushort*>(indices.data()), 3);
}
}
