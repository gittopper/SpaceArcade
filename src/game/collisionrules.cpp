...
namespace Game
{
    ...
    {
        if(!s.getBBox().isInside(getBBox() + velocity) )
        {
            cutShift(getBBox(), s.getBBox(), velocity);
        }
    }

    void Scene::visit(class IObject& obj)
    {
        if (!intersects(&obj) && Math::dot(obj.getShift(),obj.getV())>=0)//not in scene and moving away
        {
    ...
    {
        bool collide = game->getGameConfig()->collideWithAsteroidParts;
...
*/
    ...
    //this is algorithm to determine whether is the point inside the unconvex hull or not
